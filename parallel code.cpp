...
#pragma omp parallel for reduction(+ : m_gradient, b_gradient) threads_num(4)
        for (int i = 0; i < n; ++i)
        {
...
int main()
{
    ...
    int dataset_size = 10000; // Increase this if you want to make the code take more time
    vector<double> x(dataset_size);
    vector<double> y(dataset_size);

    // Random data generation for x (hours studied) and y (exam scores)
    random_device rd;
    mt19937 gen(42);                               // Use a random seed for each run
